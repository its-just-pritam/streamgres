...
@Slf4j
public class DistinctPartitionReader extends BasePartitionReader {


    public DistinctPartitionReader(String topicName, Integer partition, Long offset, Integer limit) {
        ...
        this.readerQuery = String.format("""
            ...
            latest_per_key AS (
            SELECT DISTINCT ON (msg_key) *
            FROM %s
            WHERE timestamp >= (SELECT ts FROM boundary_time)
            AND msg_offset > %s
            ORDER BY msg_key, msg_offset DESC
            )
            SELECT *
            FROM latest_per_key
